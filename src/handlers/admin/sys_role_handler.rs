...
// 获取角色列表
#[get("/roles")]
pub async fn get_roles(app_state: web::Data<globals::AppState>) -> impl Responder {
    let result = sys_role_services::get_roles(&*app_state.mysql_conn)
        .await
        .map(|roles| {
            roles
                .into_iter()
                .map(|role| sys_role_dto::RoleDto {
                    id: Some(role.id),
...
pub async fn update_role(
    ...
    if let Err(e) = role_update_dto.0.validate() {
        return create_response!(Err::<Option<sys_role_dto::RoleUpdateRespDto>, ApiError>(
