        ...
        B: 'static,
{
    ...
    fn new_transform(&self, service: S) -> Self::Future {
        ready(Ok(PermissionCheckMiddleware { service:Arc::new(service) }))
    }
}
                    ...
                    match get_menus_by_role_id(&*mysql_conn, role_ids).await {
                        Ok(permissions) => {
                            ...
                            if allowed {
                                ...
                                Err(create_error_response("Authorization Failed", StatusCode::UNAUTHORIZED))
                            }
                        },
                        Err(_) => Err(create_error_response("Authorization Failed", StatusCode::UNAUTHORIZED)),
                    }
                },
                Err(_) => Err(create_error_response("Authorization Failed", StatusCode::UNAUTHORIZED)),
            }
